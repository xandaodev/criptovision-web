...
function Dashboard() {
  ...
  // --- ESTADOS PARA O MODAL DE APORTE ---
  ...
  useEffect(() => {
    carregarTudo(); // Agora chamamos a função que carrega as duas coisas
  }, []);
  ...
  const handleEliminarTransacao = async (id) => {
    const confirmacao = window.confirm("Tem certeza que deseja excluir esta transação? Seu saldo será recalculado.");
  ...
  const handleNovoAporte = async (e) => {
    ...
    try {
      ...
      if (resposta.ok) {
        ...
        alert("Atenção: " + erroData.mensagem);
      }
    } catch (erro) {
    ...
    <div className="min-h-screen bg-gray-900 text-white p-8 font-sans relative">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8 border-b border-gray-700 pb-4">
          ...
          <button 
            onClick={fazerLogout}
            className="bg-gray-800 border border-gray-700 hover:bg-red-600 hover:border-red-600 px-6 py-2 rounded-md font-semibold transition duration-300"
          >
            Sair
          </button>
        </div>
        ...
        {carregando ? (
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
              {/* Card 1 */}
              <div className="bg-gray-800 rounded-xl p-6 shadow-lg border border-gray-700 hover:border-blue-500 transition duration-300">
              ...
              {/* Card 2 */}
              <div className="bg-gray-800 rounded-xl p-6 shadow-lg border border-gray-700 hover:border-yellow-500 transition duration-300">
                <h3 className="text-gray-400 text-sm font-medium mb-2">Preço Médio Alvo (BTC)</h3>
                <p className="text-3xl font-bold text-yellow-400">
                  $ 80.000,00
                </p>
                <p className="text-gray-400 text-sm mt-2">Monitoramento contínuo da carteira principal</p>
              </div>
              {/* Card 3 */}
            ...
            <div className="bg-gray-800 rounded-xl shadow-lg border border-gray-700 overflow-hidden mb-10">
              <div className="p-6 border-b border-gray-700 flex justify-between items-center">
                <h2 className="text-xl font-bold text-gray-200">Meus Ativos</h2>
                <button 
                  onClick={() => setModalAberto(true)}
                  className="bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold py-2 px-4 rounded transition duration-200"
                >
                  + Novo Aporte
                </button>
              </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-gray-900 text-gray-400 text-sm uppercase tracking-wider">
                      <th className="p-4 font-medium">Ativo (Ticker)</th>
                      <th className="p-4 font-medium">Saldo (Qtd)</th>
                    ...
                    {ativos.length > 0 ? (
                      ativos.map((ativo, index) => (
                        <tr key={index} className="hover:bg-gray-750 transition duration-150">
                          ...
                          <td className="p-4 text-gray-300">$ {ativo.precoAtual.toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
                          <td className="p-4 font-bold text-blue-400">$ {ativo.valorTotalUSD.toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
                      ...
                      <tr>
                        <td colSpan="5" className="p-8 text-center text-gray-500">
                          Sua carteira está vazia. Cadastre sua primeira transação clicando em "+ Novo Aporte"!
                        </td>
                      </tr>
                    )}
                  </tbody>
            ...
            {/* O SEU COMPONENTE DE EXTRATO FICA AQUI DENTRO DO MAX-W-6XL */}
            <Extrato 
               historico={historico} 
               formatarData={formatarData} 
               onEliminar={handleEliminarTransacao} 
            />
      ...
      {modalAberto && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50">
          <div className="bg-gray-800 p-8 rounded-xl shadow-2xl border border-gray-700 w-full max-w-md">
            <h2 className="text-2xl font-bold text-white mb-6">Registrar Compra</h2>
            
            <form onSubmit={handleNovoAporte} className="space-y-4">
              <div>
                <label className="block text-gray-400 text-sm mb-1">Moeda (Ticker)</label>
                <input 
                  type="text" required placeholder="Ex: BTC, ETH, SOL"
                  className="w-full bg-gray-900 border border-gray-700 text-white rounded p-3 uppercase focus:outline-none focus:border-blue-500"
                  value={novoTicker} onChange={(e) => setNovoTicker(e.target.value)}
                />
              </div>

              <div>
                <label className="block text-gray-400 text-sm mb-1">Tipo de Operação</label>
                <select 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    type="number" step="any" required placeholder="Ex: 0.05"
                    className="w-full bg-gray-900 border border-gray-700 text-white rounded p-3 focus:outline-none focus:border-blue-500"
                    value={novaQuantidade} onChange={(e) => setNovaQuantidade(e.target.value)}
                  />
                </div>
                <div>
                  <label className="block text-gray-400 text-sm mb-1">Preço Unitário ($)</label>
                  <input 
                    type="number" step="any" required placeholder="Ex: 65000"
                    className="w-full bg-gray-900 border border-gray-700 text-white rounded p-3 focus:outline-none focus:border-blue-500"
              ...
              <div className="flex justify-end space-x-3 mt-8">
                ...
                <button 
                  ...
                  className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded font-bold transition duration-200 flex items-center"
                >
                  {salvandoAporte ? 'Salvando...' : 'Salvar Aporte'}
                </button>
              </div>
            </form>
          </div>
